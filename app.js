...
document.getElementById('tax-rate').innerText = taxRate *100;
addBtn.addEventListener('click', () => { 
    ...
    totalAmount.innerText = parseInt(subTotal.innerText) + (parseInt(subTotal.innerText)*taxRate);
});
...
btnCheckOut.addEventListener('click', (e) => {
    let date = new Date();
    document.getElementById('display-date').innerText = `${date.getDate()} - ${(date.getMonth()+1)< 10 ? '0'+(date.getMonth()+1) : (date.getMonth()+1)} - ${date.getFullYear()}:: ${date.toTimeString()}`;
    let customerNumber = parseInt(phoneNumber.value);
    if ((!customerNumber) || (addedProduct <= 0 && addedCase <= 0 )) {
        alert('Invalid Mobile Number Provided/No items in the cart');
    } else {
        ...
        document.getElementById('tax').innerHTML = taxRate *100;
